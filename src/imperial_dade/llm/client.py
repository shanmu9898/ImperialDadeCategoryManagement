...
from imperial_dade.llm.azure_config import (
    ...
    def submit_batch(self, file_paths: List[str]) -> List[str]:
        ...
        while any([status_dict[k] not in ("processed") for k in file_ids]):
            for file_id in file_ids:
                ...
                logger.info(f"{datetime.datetime.now()} File Id: {file_id},  Status: {status}")
            time.sleep(20)
        logger.info(f"Files uploaded with ids: {file_ids}")
    ...
    def retrieve_batches(self, batch_ids: List[str]) -> List[dict]:
        ...
        for batch_id, status in status_dict.items():
            if status == "completed":
                ...
                logger.info(f"Retrieved: {len(responses)}")
            elif status == "failed":
                logger.info(f"Batch {batch_id} failed")
    ...
    def get_summary(self, reviews: List[str], batch_trigger: int = 30000, **model_kwargs) -> str:
        ...
        for i in range(0, levels):
            ...
            if len(new_summaries) == 1:
                if i != levels - 1:
                    logger.warning(f"Warning: Only one response received at level {i}. Last layer wasn't executed")
                break
