...
def _validate_write_matches_inputs(
    ...
    if missing_cols:
        logger.warning(f"Warning: Missing essential columns: {missing_cols}")
...
def _clean_dataframe_columns(
    ...
    if item_id_col in df.columns:
        ...
        logger.warning(f"Warning: '{item_id_col}' not found, cannot perform cleaning on it.")
...
def _ensure_essential_columns(
    ...
    for col_name in essential_cols:
        if col_name not in df.columns:
            logger.warning(f"Warning: Column '{col_name}' not found in input DataFrame. It will be treated as empty/default.")
...
def _get_description_column(
    ...
    if main_desc_col not in df.columns or df[main_desc_col].isnull().all():
        logger.warning(f"Warning: Specified main description column '{main_desc_col}' not found or is all null. Falling back to '{fallback_desc_col}'.")
        if fallback_desc_col not in df.columns or df[fallback_desc_col].isnull().all():
            logger.warning(f"Warning: Fallback description column '{fallback_desc_col}' also not found or is all null. Descriptions will be 'N/A'.")
...
def _parse_model_matches(raw_model_match_data: Any) -> List[str]:
    ...
    if isinstance(raw_model_match_data, str):
        try:
            ...
            logger.warning(f"Warning: Could not parse '{raw_model_match_data}' as a list. Setting to empty list.")
            model_match_ids = []
...
def _create_target_data(
    ...
    target_data = {
        ...
        'is_target': True,
        '_row_type': 'Target',
        '_raw_vb_flag': item_row.get(pl_flag_col, 'N') if pl_flag_col in item_row else 'N',
...
def _create_match_data(
    ...
    if enable_vpn_exclusion:
        ...
        if _should_exclude_vpn_match(match_vpn_code, target_vpn_code, target_item_code, enable_vpn_exclusion):
            logger.info(f"Skipping match {match_entity_item_id} due to VPN matching conditions")
            return None
    ...
    match_info['_raw_vb_flag'] = match_series.get(pl_flag_col, 'N') if pl_flag_col in match_series else 'N'
...
def write_top_matches(
    ...
    Only non-VB items (VB Flag == 'N') are written as target sheets. VB and Y-Other items
    can still appear as substitutes within those sheets. Items with no valid substitutes
    are skipped entirely and do not appear in the output file.
    ...
    Args:
        ...
        pl_flag_col: Column indicating VB flag status ('N', 'Y - VB', 'Y - Other')
        vpn_code_col: Column for VPN codes (used only if enable_vpn_exclusion=True)
        pl: Retained for backwards compatibility, no longer used for filtering
        enable_vpn_exclusion: If True, enable VPN code exclusion logic and include VPN-related columns
    """
    logger.info(f"Starting write_top_matches with enable_vpn_exclusion={enable_vpn_exclusion}")

    # VB Flag value identifying a Victoria Bay item (vs 'N' / 'Y - Other')
    VB_FLAG_VALUE = 'Y - VB'
    ...
    im_final_full = _clean_dataframe_columns(im_final_full2, item_id_col, vpn_code_col, item_code_col,
                                             enable_vpn_exclusion)
    ...
    # Calculate totals across the full dataset (all items, not just non-VB)
    total_category_net_cost = im_final_full[
        net_cost_col].sum() if net_cost_col in im_final_full.columns and pd.api.types.is_numeric_dtype(
        im_final_full[net_cost_col]) else 0.0
    total_category_qty = im_final_full[
        qty_col].sum() if qty_col in im_final_full.columns and pd.api.types.is_numeric_dtype(
        im_final_full[qty_col]) else 0.0

    # Get top N items — restricted to non-VB items only (VB Flag == 'N')
    # VB and Y-Other items are never targets; they can still appear as substitutes
    top_n_items = pd.DataFrame()
    logger.info(f"Looking for qty column: '{qty_col}'")
    logger.info(f"Available columns: {list(im_final_full.columns)}")

    if not im_final_full.empty and qty_col in im_final_full.columns:
        logger.info(f"Found qty column '{qty_col}' with {len(im_final_full)} rows")
        if pd.api.types.is_numeric_dtype(im_final_full[qty_col]):
            logger.info(f"Qty column is numeric, sorting by {qty_col}")

            # Filter to non-VB targets only
            if pl_flag_col in im_final_full.columns:
                non_vb_df = im_final_full[im_final_full[pl_flag_col] == 'N']
                logger.info(
                    f"Non-VB filter applied (VB Flag == 'N'): {len(non_vb_df)} of {len(im_final_full)} rows qualify as targets")
            else:
                logger.warning(f"Warning: VB Flag column '{pl_flag_col}' not found, no VB filtering applied")
                non_vb_df = im_final_full

            top_n_items = non_vb_df.sort_values(by=qty_col, ascending=False).head(n)
            logger.info(f"Selected top {len(top_n_items)} non-VB items by Qty")
        else:
            logger.warning(
                f"Warning: Qty column '{qty_col}' is not numeric. Available data types: {im_final_full[qty_col].dtype}")
    else:
        logger.warning(f"Warning: Qty column '{qty_col}' not found in DataFrame")

    if top_n_items.empty:
        logger.info(f"No non-VB items found for 'Top {n}' section. Individual sheets will not be created.")
        return
    ...
    try:
        with pd.ExcelWriter(base_filename, engine='xlsxwriter') as writer:
            ...
            # Process each top non-VB target item
            for index, item_row in top_n_items.iterrows():
                ...
                # Collect valid matches in model-ranked order (best first)
                collected_matches = []
                for match_entity_item_id in model_match_ids:
                    ...
                    collected_matches.append(match_info)

                # Split substitutes: every non-VB substitute, plus AT MOST ONE
                # Victoria Bay ('Y - VB') substitute — the highest-ranked one.
                # Targets are always non-VB; it is OK for there to be no VB option.
                # 'Y - Other' private-label items remain regular substitutes.
                non_vb_subs = [m for m in collected_matches if m.get('_raw_vb_flag') != VB_FLAG_VALUE]
                vb_candidates = [m for m in collected_matches if m.get('_raw_vb_flag') == VB_FLAG_VALUE]
                best_vb_sub = vb_candidates[0] if vb_candidates else None
                if best_vb_sub is not None:
                    best_vb_sub['_row_type'] = 'VB Substitute'

                substitute_items_data_for_current_target = (
                    non_vb_subs + ([best_vb_sub] if best_vb_sub else [])
                )

                # Build table rows (target first, then substitutes) and attribute keys
                all_items_for_table_data = [target_data] + substitute_items_data_for_current_target
                all_attribute_keys_for_table = set(target_data['attributes'].keys())
                for m in substitute_items_data_for_current_target:
                    all_attribute_keys_for_table.update(m['attributes'].keys())

                # Skip sheet entirely if no valid substitutes found
                if not substitute_items_data_for_current_target:
                    logger.info(f"Skipping target item: {target_data[item_id_col]} — no valid substitutes found")
                    continue

                # Accumulate target metrics only for items that have substitutes
                ...
                for match_info in substitute_items_data_for_current_target:
                ...
                logger.info(
                    f"Processed target item: {target_data[item_id_col]} with {len(substitute_items_data_for_current_target)} substitutes")
            ...
            logger.info(f"Excel report successfully created: {base_filename}")

    except ImportError:
        logger.info("xlsxwriter not found, falling back to openpyxl. Formatting will be basic, summary will be static.")
        try:
            with pd.ExcelWriter(base_filename, engine='openpyxl') as writer:
                logger.info(f"Excel report created with basic formatting: {base_filename}")
        except Exception as e:
            logger.error(f"Error creating Excel file with openpyxl: {e}")
            return
    except PermissionError:
        logger.error(f"Error: Excel file '{base_filename}' is open in another application. Please close it and try again.")
        return
    except Exception as e:
...
def make_embeddings(
    ...
    """
    logger.info("Starting make_embeddings...")
    ...
    logger.info(f"Rows prepared for embedding: {len(im_final_full)}")
    ...
    for i in tqdm(range(0, len(descriptions), batch_size), desc="Embedding Batches"):
        ...
        try:
            ...
            logger.info(f"Batch {i // batch_size} failed: {e}")
    ...
    if len(embedded_text_list) == len(im_final_full):
        ...
        logger.info("Embedding count mismatch. Skipping embeddings column assignment.")
        return im_final_full
...
def get_user_prompts(
    ...
    if im_final_full.empty:
        logger.info("Critical: Input DataFrame is empty!")
        logger.info("Check your data loading and preprocessing steps.")
        return [], {}
...
Return a list of Entity IDs that are swappable with the given Entity ID from the list of similar items, ordered by vendor preference (Victoria Bay > Preferred Vendors > Others).
...
"""
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Generating Prompts"):
        ...
        current_vendor = row.get('VGN', '')

        if use_faiss:
            # Embeddings are already normalized
            # Increased buffer to give GPT more options for ordering
            _, sim_indices = index.search(np.expand_dims(current_emb, axis=0), n + 20)
            sim_indices = sim_indices[0]
        else:
            sims = cosine_similarity(current_emb.reshape(1, -1), embeddings)[0]
            sim_indices = sims.argsort()[::-1][:n + 20]
        ...
        for sim_idx in sim_indices:
            ...
            if vendor_exclusion and candidate.get('VGN', '') == current_vendor:
                continue
            ...
            candidates.append({
                ...
                'vgn_name': candidate.get('VGN', ''),
                'levenshtein_sim': lev_score
            })
            # Collect more candidates since GPT will do the ordering and filtering
...
def generate_matches(
    ...
    if duplicate_mapping is not None:
        ...
        logger.info(f"Expanded to {len(responses)} responses")
...
def _create_system_prompt(category: CategoryConfig, hard_rules: Optional[str] = None) -> str:
    """Render the matching system prompt for a given category.

    All category-specific content (critical attributes, exclusion rules,
    vendor hierarchies) lives in the Jinja template + the CategoryConfig.
    """
...
def _process_ai_responses(
    ...
    for i, json_str in enumerate(responses):
        if json_str is None:
            logger.warning(f"Warning: No AI response received for prompt index {i}. Skipping.")
            df.at[i, 'Matches'] = []
        ...
        if not isinstance(json_str, str):
            logger.error(f"Error: Response {i} is not a string. Type: {type(json_str).__name__}")
            df.at[i, 'Matches'] = []
        ...
        try:
            ...
            logger.error(f"Error decoding JSON response {i}: {e}")
            logger.info(f"Response preview: {json_str[:200]}...")
            df.at[i, 'Matches'] = []
            df.at[i, 'reasoning'] = f"JSON decode error: {str(e)}"
        except Exception as e:
            logger.info(f"Unexpected error processing response {i}: {e}")
...
def _process_single_response(
    ...
    if responded_numerical_eid is None or not isinstance(responded_numerical_eid, int):
        logger.warning(f"Warning: Invalid or missing 'Entity ID' in AI response index {response_index}. Skipping.")
        return
    ...
    if original_entity_id is None:
        logger.warning(f"Warning: Responded numerical EID {responded_numerical_eid} not found in mapping. Skipping.")
        return
...
def _process_matches_list(
    ...
    if not isinstance(numerical_matches_list, list):
        logger.warning(f"Warning: 'Matches' field in response {response_index} is not a list. Defaulting to empty.")
    ...
    for num_id in numerical_matches_list:
        ...
        if mapped_str:
            ...
            logger.warning(f"Warning: Numerical match ID {num_id} not found in mapping. Skipping match.")
...
def _update_dataframe_with_matches(
    ...
    if not matched_indices.empty:
        ...
        logger.warning(f"Warning: Original Entity ID '{entity_id}' not found in DataFrame.")
...
def _create_individual_worksheet(
    ...
    for df_row_idx_loop, item_d in enumerate(all_items_for_table_data):
        ...
        rec_f = target_fmt_thick_right if is_tgt else cell_fmt_thick_right
        ...
        c_idx_write += 1
        worksheet.write(excel_data_row, c_idx_write, item_d.get('_row_type', 'Substitute'), rec_f)
        c_idx_write += 1
...
def run(
    im_final: pd.DataFrame,
    agent,
    category,                          # CategoryConfig (annotated as-is to avoid runtime import order issues)
    output_dir,                        # pathlib.Path
    hard_rules: Optional[str] = None,
    batch_model: bool = False,
    pl: bool = True,
    enable_vpn_exclusion: bool = True,
    n: Optional[int] = None,
) -> pd.DataFrame:
    """End-to-end matching phase.

    Replaces the manual sequence in legacy 02_Suggested_Subs.ipynb:
        make_embeddings -> get_user_prompts -> generate_matches -> write_top_matches

    The category-specific system prompt is rendered by `generate_matches` via
    `_create_system_prompt(category)` (Jinja2 + CategoryConfig).

    Args:
        n: top-N items to include in the Excel report. Defaults to
           `category.matching.top_n`.
    """
    ...
    if n is None:
        n = category.matching.top_n

    logger.info("Matching stage: %d rows, top_n=%d, batch=%s",
                len(im_final), n, batch_model)

    im_final = make_embeddings(im_final)
    user_prompts, idx_map = get_user_prompts(im_final)
    im_final = generate_matches(
        im_final,
        user_prompts,
        idx_map,
        category=category,
    ...
    out_path = output_dir / f"{category.name}_Subs_{date.today():%Y-%m-%d}.xlsx"
    write_top_matches(
        im_final,
        str(out_path),
        n=n,
        pl=pl,
        enable_vpn_exclusion=enable_vpn_exclusion,
    )
    logger.info("Matching stage complete: wrote %s", out_path)
    return im_final
