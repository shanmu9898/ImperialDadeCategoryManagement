...
def consolidate_feedback_from_excel(
    ...
    for i, excel_filepath in enumerate(excel_filepaths):
        ...
        try:
            ...
            logger.info(f"  [ERROR] Could not open or read file: {e}. Skipping.")
            continue

        all_sheet_names_in_file = xls.sheet_names
        logger.info(f"  [INFO] Found {len(all_sheet_names_in_file)} sheets.")

        for sheet_name in all_sheet_names_in_file:
            if sheet_name in sheets_to_ignore:
                logger.info(f"    - Skipping ignored sheet: '{sheet_name}'")
                continue
            ...
            try:
                ...
                if df_sheet.empty:
                    logger.info(f"    - [WARNING] Sheet '{sheet_name}' is empty. Skipping.")
                    continue

                if 'Entity--Item' not in df_sheet.columns:
                    logger.info(f"    - [WARNING] 'Entity--Item' column not found in sheet '{sheet_name}'. Skipping.")
                    continue
                
                if len(df_sheet) < 2:
                    logger.info(f"    - [WARNING] Sheet '{sheet_name}' has a target but no substitute rows. Skipping.")
                    continue
                ...
                for idx, sub_row in df_sheet.iloc[1:].iterrows():
                    ...
                    if pd.isna(sub_item_id_val) or (isinstance(sub_item_id_val, str) and not sub_item_id_val.strip()):
                        logger.info(f"    - [INFO] Blank row found. Concluding substitute processing for '{sheet_name}'.")
                        break
                ...
                logger.info(f"    - [SUCCESS] Processed sheet '{sheet_name}', found {subs_found_in_sheet} substitutes for target '{target_item_id}'.")

            except Exception as e:
                logger.info(f"    - [ERROR] Failed to process sheet '{sheet_name}': {e}. Skipping.")
                continue
            
    logger.info("\n--- Consolidation Summary ---")
    if not consolidated_data:
        logger.info("[WARNING] No substitute data was consolidated from any file.")
        return pd.DataFrame()
        
    logger.info(f"[INFO] Files processed: {files_processed_count}/{total_files}")
    logger.info(f"[INFO] Sheets processed: {sheets_processed_count}")
    logger.info(f"[INFO] Total substitute records found: {len(consolidated_data)}")
    
    feedback_summary_df = pd.DataFrame(consolidated_data)
    
    logger.info("[INFO] Merging feedback data with item master for embeddings...")
    ...
    logger.info("[SUCCESS] Merging complete.")
    logger.info("--------------------------------\n")
...
def summarize_prompt_into_rules(prompt_text: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error during LLM call or parsing: {e}")
        return ""
    return ""
...
def user_prompts_with_rules(
    ...
    if use_subcategory_rules:
        logger.info("Starting user_prompts_with_rules function with subcategory-specific rule generation...")
    else:
        logger.info("Starting user_prompts_with_rules function with unified rule generation...")
    ...
    if embedding_col not in im_final_full.columns:
        logger.info(f"Critical: Neither 'embeddings' nor 'attributes' column found in DataFrame.")
    ...
    if im_final_full.empty:
        logger.info(f"Critical: No valid embeddings in '{embedding_col}' column.")
        return [], {}
    ...
    if only_rejects and isinstance(merged_df, pd.DataFrame) and not merged_df.empty:
        ...
        logger.info(f"Filtered feedback to only rejects: {reject_feedback_count}/{original_feedback_count} records")
        
        if merged_df.empty:
            logger.warning("Warning: No reject feedback found. Proceeding with empty rules.")
    elif only_rejects:
        logger.warning("Warning: only_rejects=True but no feedback data available. Proceeding with empty rules.")
    ...
    if use_subcategory_rules:
        ...
        if isinstance(merged_df, pd.DataFrame) and not merged_df.empty and subcategory_col in merged_df.columns:
            logger.info(f"Generating subcategory-specific rules from feedback examples...")
            
            # Get unique subcategories
            unique_subcategories = merged_df[subcategory_col].dropna().unique()
            logger.info(f"Found {len(unique_subcategories)} unique subcategories: {unique_subcategories}")
            ...
            for subcategory in unique_subcategories:
                ...
                if not subcategory_feedback.empty:
                    ...
                    if subcategory_feedback_prompt_lines:
                        ...
                        logger.info(f"Generated {rule_count} rules for subcategory: {subcategory}")
                else:
                    logger.info(f"No feedback data found for subcategory: {subcategory}")
        else:
            logger.warning(f"Warning: No feedback data or '{subcategory_col}' column found. Using empty subcategory rules.")
        ...
        if isinstance(merged_df, pd.DataFrame) and not merged_df.empty:
            logger.info("Generating one unified set of rules from all feedback examples...")
            ...
            if all_feedback_prompt_lines:
                ...
                logger.info("Unified rule generation complete.")
...
"""
    ...
    for idx, row in tqdm(im_final_full.iterrows(), total=im_final_full.shape[0], desc="Generating Prompts"):
        ...
        if use_subcategory_rules:
            ...
            if current_subcategory and current_subcategory in subcategory_rules_dict:
                ...
                logger.info(f"Using {rule_count} rules for subcategory: {current_subcategory}")
            else:
                rules_for_prompt = ""
                if current_subcategory:
                    logger.info(f"No rules found for subcategory: {current_subcategory}")
                else:
                    logger.info(f"No subcategory found for item: {current_item_id}")
        else:
    ...
    logger.info(f"Generated {len(user_prompts)} prompts.")
...
def update_matches_based_on_feedback_sym(
    ...
    """
    logger.info("Starting update with strict target-centric rules...")
    ...
    logger.info(f"Identified {len(all_targets_in_feedback)} unique Targets in feedback.")
    ...
    logger.info(f"Processing {len(feedback_df_cleaned)} feedback entries to build allowed pairs...")
    ...
    logger.info("Applying conditional filtering (Pass 1 of 2)...")
    ...
    logger.info("Reconciling matches for global symmetry (Pass 2 of 2)...")
    ...
    logger.info("Finished update with strict target-centric rules.")
...
def run(
    ...
    """End-to-end feedback phase.

    Replaces the manual sequence in legacy 03_Feedback.ipynb:
        consolidate_feedback_from_excel
        get_feedback_summary  (logged)
        get_reviewed_coverage (logged)
        normalize accept/reject labels (per cfg.feedback)
        user_prompts_with_rules
        matching.generate_matches  (with hard_rules)
        update_matches_based_on_feedback_sym
        matching.write_top_matches

    Returns:
        (im_final2, im_final_with_feedback) — final matches + consolidated feedback.
    """
    from datetime import date
    from pathlib import Path

    from imperial_dade.stages.matching import generate_matches, write_top_matches

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    accept = category.feedback.accept_label
    reject = category.feedback.reject_label

    logger.info("Feedback stage: %d items, %d feedback files, labels=(%s, %s)",
                len(im_final), len(feedback_files), accept, reject)

    # Step 1 — consolidate Excel sheets into a feedback frame
    im_final_with_feedback = consolidate_feedback_from_excel(feedback_files, im_final)

    # Steps 2–3 — log-only summary + reviewed coverage
    try:
        get_feedback_summary(im_final_with_feedback)
        get_reviewed_coverage(im_final, im_final_with_feedback)
    except Exception as exc:
        logger.warning("feedback summary/coverage skipped (%s)", exc)

    # Step 4 — normalize Accept/Consider/Reject -> category-specific labels
    if "Correct" in im_final_with_feedback.columns:
        im_final_with_feedback = im_final_with_feedback.copy()
        im_final_with_feedback["Correct"] = im_final_with_feedback["Correct"].replace({
            "Accept": accept,
            "Consider": accept,
            "Reject": reject,
        })

    # Step 5 — regenerate prompts with the learned rules
    user_prompts, idx_map = user_prompts_with_rules(
        im_final,
        im_final_with_feedback,
        use_subcategory_rules=category.feedback.use_subcategory_rules,
    )

    # Step 6 — re-run matching with the feedback-informed hard rules
    im_final_test = generate_matches(
        im_final,
        user_prompts,
        idx_map,
        category=category,
    ...
    # Step 7 — apply symmetry-aware feedback constraints
    im_final2 = update_matches_based_on_feedback_sym(
        im_final_test,
        im_final_with_feedback,
        feedback_correct_col="Correct",
        accept_value=accept,
        reject_value=reject,
    )

    # Step 8 — final Excel deliverable
    out_path = output_dir / f"{category.name}_Subs_{date.today():%Y-%m-%d}_post_feedback.xlsx"
    write_top_matches(
        im_final2,
        str(out_path),
    ...
    logger.info("Feedback stage complete: wrote %s", out_path)

    return im_final2, im_final_with_feedback
