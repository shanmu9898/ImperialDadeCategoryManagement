...
def _determine_excel_engine(output_excel_filepath: str) -> str:
    ...
    try:
        ...
        logger.info("xlsxwriter not found, falling back to openpyxl. Formatting will be basic.")
        return 'openpyxl'
    except Exception as e_init:
        logger.info(f"Initial xlsxwriter test failed: {e_init}. Falling back to openpyxl.")
        return 'openpyxl'
...
def _prepare_dataframe_for_report(df: pd.DataFrame, required_cols: List[str]) -> pd.DataFrame:
    ...
    if missing_cols:
        logger.warning(f"Warning: DataFrame is missing expected columns: {', '.join(missing_cols)}. Adding with NaN values.")
...
def _create_suggested_swaps_sheet(writer, workbook, df: pd.DataFrame, volume_redistribution_dict: Dict,
    ...
    if not volume_redistribution_dict:
        logger.info("volume_redistribution_dict is empty. No swaps to list on this sheet.")
...
def _create_swap_details_dataframe(df: pd.DataFrame, volume_redistribution_dict: Dict,
    ...
    if not df[kwargs.get('item_id_col', 'Entity--Item')].is_unique:
        logger.warning(f"Warning: '{kwargs.get('item_id_col', 'Entity--Item')}' is not unique in the input DataFrame. Using first occurrence for lookups in 'Suggested Swaps'.")
    ...
    if swap_details_list:
        ...
        logger.info("No valid swaps to list after processing volume_redistribution_dict.")
        suggest_swaps_df_final = pd.DataFrame(columns=swap_output_columns)
...
def _create_suggested_swaps_sheet_new(writer, workbook, updated_df: pd.DataFrame, 
    ...
    if swaps_df.empty:
        logger.info("No swaps found in updated_df. Creating empty sheet.")
...
def create_enhanced_sku_report(
    ...
    """
    logger.info(f"--- Starting to generate Enhanced SKU Report: {output_excel_filepath} ---")
    ...
    try:
        ...
        logger.info(f"--- Successfully created Enhanced SKU Report: {output_excel_filepath} ---")
        return True

    except Exception as e:
        logger.error(f"Error creating Enhanced SKU Report: {str(e)}")
        return False
...
def read_and_process_feedback_excel(
    ...
    """
    logger.info(f"--- Reading feedback from Excel file: {excel_filepath} ---")

    if not os.path.exists(excel_filepath):
        logger.error(f"Error: Excel file not found at {excel_filepath}")
        return None
    ...
    if suggest_swaps_sheet_name in xls.sheet_names:
        logger.info(f"Processing sheet: {suggest_swaps_sheet_name}...")
        try:
            ...
            if missing_swaps_cols:
                logger.warning(f"Warning: Sheet '{suggest_swaps_sheet_name}' is missing columns: {', '.join(missing_swaps_cols)}. Skipping this sheet for detailed extraction.")
                suggested_swaps_df = pd.DataFrame(columns=required_swaps_cols)
            else:
                suggested_swaps_df = df_swaps[required_swaps_cols].copy()
                logger.info(f"Successfully extracted data from '{suggest_swaps_sheet_name}'. Found {len(suggested_swaps_df)} rows.")

        except Exception as e:
            logger.error(f"Error processing sheet '{suggest_swaps_sheet_name}': {e}")
        ...
        logger.warning(f"Warning: Sheet '{suggest_swaps_sheet_name}' not found in the Excel file.")
        suggested_swaps_df = pd.DataFrame(columns=[suggest_swaps_orig_sku_col, suggest_swaps_recv_sku_col, accept_reject_col, feedback_col])
