...
def write_top_matches(
    ...
    if not im_final_full.empty and qty_col in im_final_full.columns:
        ...
        if pd.api.types.is_numeric_dtype(im_final_full[qty_col]):
            ...
            if pl_flag_col in im_final_full.columns:
                ...
                print(f"Warning: VB Flag column '{pl_flag_col}' not found, no VB filtering applied")
                non_vb_df = im_final_full

            top_n_items = non_vb_df.sort_values(by=qty_col, ascending=False).head(n)
            print(f"Selected top {len(top_n_items)} non-VB items by Qty")
        else:
            print(
    ...
    if top_n_items.empty:
        print(f"No non-VB items found for 'Top {n}' section. Individual sheets will not be created.")
        return
        ...
        with pd.ExcelWriter(base_filename, engine='xlsxwriter') as writer:
            ...
            for index, item_row in top_n_items.iterrows():
                ...
                if not substitute_items_data_for_current_target:
                    print(f"Skipping target item: {target_data[item_id_col]} — no valid substitutes found")
                    continue
...
You will be given an item and its attributes. You will also be given a list of similar items.

Please pay close attention to the attribute fields below 'Description'. The more of these fields 
that have matching or similar values between your original item and potential substitute items, 
the better the chance that they are a true match.

CRITICAL MATCHING REQUIREMENTS (MUST MATCH EXACTLY):
The following attributes MUST match exactly when the information is available. If any of these 
don't match, the items are NOT swappable under any circumstances:

1. Product Capacity (e.g., 12 oz, 16 oz, 20 oz) — capacities must be identical.
   Ignore items where this field is "null".

2. Beverage Cup Type (e.g., cup, lid, combo, cone) — types must be identical.
   Ignore items where this field is "null".

3. Usage Temperature (e.g., hot cup, cold cup) — must be identical.
   Ignore items where this field is "null".

4. Color — must match. Do NOT suggest a white cup as a substitute for a black cup or any 
   other color mismatch. If Color is available and differs between items, they are NOT swappable.
   Ignore items where this field is "null".

5. Beverage Cup Style — must match exactly. This field captures wall construction and product 
   line, for example:
   - "Double Wall" and "Single Wall" cups are NOT interchangeable.
   - "Perfect Touch" is a distinct style with NO valid substitutes. If the original item has 
     Beverage Cup Style = "Perfect Touch", return an empty Matches list immediately.
   - Do not suggest a "Perfect Touch" item as a substitute for any non-Perfect Touch item.
   If Beverage Cup Style is available and differs between items, they are NOT swappable.
   Ignore items where this field is "null".


ABSOLUTE EXCLUSIONS:
- Perfect Touch: If the original item's Beverage Cup Style is "Perfect Touch", return an 
  empty Matches list. No exceptions.
- Victoria Bay same-brand rule: The original item's VB Flag will be shown to you. 
  If VB Flag = 'Y - VB', do NOT suggest any other Victoria Bay products as substitutes. 
  Victoria Bay items (VB Flag = 'Y - VB') should only be matched with non-Victoria Bay alternatives.

DIRECTIONAL MATCHING (SIMILAR IS ACCEPTABLE, NOT REQUIRED):
The following attributes should be similar but do not need to be exact matches:
- Material — comparable quality/type is acceptable
- Beverage Cup Style — where not null, prefer matching styles after the critical rules above are satisfied
- Product Dimensions, Product Dimension Type, Bottom Diameter (IN), Top Diameter (IN) — 
  should be within a reasonable range
- Pattern & Design — similar style is acceptable
- Pack Size, Sustainable Products, Compatible Product & Product Type, 
  Foodservice Global Attributes — consider but not disqualifying

Please ignore fields where the result is "null". Two values of "null" are NOT considered a match.

PREFERRED VENDOR HIERARCHY:
When ordering your final Matches list, prioritize in this order:
1. Victoria Bay (VB Flag = 'Y - VB') products — prioritize these first.
   EXCEPTION: If the original item itself has VB Flag = 'Y - VB', skip this tier entirely and 
   do not suggest any other Victoria Bay products.   
2. Preferred vendors by product type:
   - Paper Hot Cup: Graphic Packaging, Dart, Eco Products, World Centric, Georgia Pacific, Huhtamaki
   - Paper Cold Cup: Continental Cup, Graphic Packaging, Dart
   - Plastic Cold Cup: Dart, Fabrikal
   - Plastic Portion Cup: Dart, Fabrikal, World Centric, Eco Products
3. All other vendors

SUMMARY OF RULES:
1. All five CRITICAL attributes must match (or both be null) — Capacity, Cup Type, 
   Temperature, Color, Beverage Cup Style.
2. Perfect Touch originals → empty Matches list, no exceptions.
3. Perfect Touch items → never appear in any Matches list.
4. VB Flag = 'Y' originals → no other VB products in Matches.
5. Order final Matches by vendor hierarchy (VB first unless original is VB, then preferred, then others).
6. If nothing passes all critical checks, return an empty Matches list.

{hard_rules_str}

Respond in JSON format with the following fields:
- Entity ID: The numerical Entity ID of the item you are evaluating.
- Matches: Ordered list of numerical Entity IDs that are valid substitutes, meeting all 
  critical requirements above. Only include IDs that were provided in the prompt.
- reasoning: Brief explanation covering which critical attributes matched, which items were 
  excluded and why (e.g. "Excluded item 42: Color mismatch — original is Black, candidate is White").
